# Novel Agent System 改进方案文档 v2.0
...
### 10.1 第一阶段：Bridge API 核心（3周）

| 任务 | 优先级 | 时间 | 依赖 |
|------|--------|------|------|
| FastAPI Bridge 框架 | P0 | 2天 | - |
| Neo4j 客户端集成 | P0 | 3天 | FastAPI |
| Chroma 客户端集成 | P1 | 2天 | FastAPI |
| 大纲层级查询端点 | P0 | 3天 | Neo4j |
| 关系图谱查询端点 | P0 | 3天 | Neo4j |
| 错误处理机制 | P0 | 2天 | FastAPI |

### 10.2 第二阶段：LSP 核心功能（4周）

| 任务 | 优先级 | 时间 | 依赖 |
|------|--------|------|------|
| 三级大纲解析器 | P0 | 4天 | - |
| documentSymbol（层级显示） | P0 | 3天 | 解析器 |
| goto_definition（大纲跳转） | P0 | 3天 | Bridge API |
| diagnostics（集成校验代理） | P0 | 4天 | 校验代理 |
| hover（角色信息+关系） | P1 | 3天 | Bridge API |
| 性能优化（缓存策略） | P1 | 3天 | - |
...
| 任务 | 优先级 | 时间 | 依赖 |
|------|--------|------|------|
| codeLens（代理按钮） | P0 | 3天 | LSP Server |
| executeCommand（命令执行） | P0 | 4天 | codeLens |
| 人工介入流程 | P0 | 3天 | executeCommand |
| 更新代理触发 | P1 | 2天 | 更新代理 |
| 重试机制 | P1 | 2天 | 校验代理 |
...
| 任务 | 优先级 | 时间 | 依赖 |
|------|--------|------|------|
| completion（智能补全） | P1 | 3天 | Bridge API |
| rename（安全重命名） | P1 | 3天 | 符号索引 |
| 关系图谱可视化 | P2 | 4天 | Neo4j |
...
│   │   ├── plot.py          # 剧情记忆
│   │   └── world.py         # 世界观记忆
│   ├── api/                 # Bridge API 模块 ★新增
│   │   ├── server.py        # FastAPI 服务器
│   │   ├── routes/          # API 路由
│   │   │   ├── outline.py   # 大纲查询
│   │   │   ├── symbols.py   # 符号查询
│   │   │   ├── relationships.py # 关系查询
│   │   │   └── agents.py    # 代理触发
│   │   └── models.py        # 数据模型
│   ├── lsp/                 # LSP 服务器模块 ★新增
│   │   ├── server.ts        # LSP 服务器入口
